...
def visualize_two_meshes(mesh1_path, mesh2_path):
    ...
    plotter = pv.Plotter()
    x_axis = pv.Line((25, 25, 0), (30, 25, 0))
    x_axis = x_axis.tube(radius=0.05)
    y_axis = pv.Line((25, 25, 0), (25, 30, 0))
    y_axis = y_axis.tube(radius=0.05)
    z_axis = pv.Line((25, 25, 0), (25, 25, 5))
    z_axis = z_axis.tube(radius=0.05)
    plotter.add_mesh(x_axis, color='red')
    plotter.add_mesh(y_axis, color='green')
    plotter.add_mesh(z_axis, color='blue')
...
def visualize_one_mesh(mesh_path):
    pv.global_theme.allow_empty_mesh = True
    mesh1 = pv.read(mesh_path)
    plotter = pv.Plotter()
    x_axis = pv.Line((25, 25, 0), (28, 25, 0))
    x_axis = x_axis.tube(radius=0.05)
    y_axis = pv.Line((25, 25, 0), (25, 28, 0))
    y_axis = y_axis.tube(radius=0.05)
    z_axis = pv.Line((25, 25, 0), (25, 25, 3))
    z_axis = z_axis.tube(radius=0.05)
    plotter.add_mesh(x_axis, color='red')
    plotter.add_mesh(y_axis, color='green')
    plotter.add_mesh(z_axis, color='blue')
